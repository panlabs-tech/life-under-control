...
| `meta-devtools` | http | o app da Meta / WhatsApp Cloud API ([ADR-0012](../adr/0012-whatsapp-borda-meta-cloud-api.md)) | OAuth no primeiro uso |
| `aws` | stdio (`uvx`) | a conta AWS, hoje só Bedrock para extração de comprovante ([ADR-0013](../adr/0013-extracao-comprovante-llm-port.md)) | credencial IAM do ambiente |

**Todo pacote é pinado por versão exata no arquivo, e nenhum em `@latest`** -- vale para os três de `npx` e para o de `uvx`. Um MCP que resolve pacote novo a cada inicialização muda de comportamento sem ninguém ter mudado nada, e a versão em que ele mudou não fica escrita em lugar nenhum.
