...
describe("editBill (Seam 1)", () => {
  ...
  it("test_edicao_preserva_a_primeira_competencia", async () => {
    const { repo, bill } = await comUmaConta()

    const editada = await editBill(
      repo,
      "h-1",
      bill.id,
      brutoValido({ primeiraCompetencia: "2030-12" }),
    )

    expect(editada.primeiraCompetencia).toBe("2020-01")
  })
})
