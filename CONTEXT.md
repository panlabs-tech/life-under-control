...
**Proposta de Lançamento** (`PaymentProposal`) — A extração de um comprovante recebido por uma borda de ingestão (ex.: o bot de WhatsApp), aguardando confirmação de uma Pessoa: propõe Conta, valor, data de pagamento e Competência, e **só vira Lançamento quando confirmada**. Não é um fato — cancelada ou expirada, não deixa efeito no domínio. _Evite_: pré-lançamento, lançamento pendente, rascunho.
